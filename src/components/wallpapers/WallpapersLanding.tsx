...
import { ChevronRight, ArrowRight, Heart, ArrowUpDown, Sparkles } from 'lucide-react';
import { WALLPAPER_TYPES } from '@/lib/data/wallpapers';
import { Product } from '@/lib/data/types';

interface WallpapersLandingProps {
  products: Product[];
}

export function WallpapersLanding({ products }: WallpapersLandingProps) {
    ...
    <div className="bg-[#FAF7F2] min-h-screen text-[#1C1917] selection:bg-[#9A7B56] selection:text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 pb-2">
        <nav className="flex items-center gap-1.5 text-[12px] text-[#78716C]">
          ...
          <span className="text-[#1C1917] font-medium">Wallpapers & Wall Coverings</span>
        </nav>
      </div>

      {/* ─── 2. SHOP BY WALLPAPER TYPE (Reference Grid Structure) ─── */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-10">
            ...
            <p className="text-[12px] uppercase tracking-widest font-semibold text-[#9A7B56] mb-1">
              Shop by Wallpaper Type
            </p>
            <p className="text-[13.5px] sm:text-[14.5px] text-[#78716C]">
              Select a wallcovering style below to view its tailored collection.
            </p>
          </div>

          {/* 5 Types: 5 per row Desktop, 3 Tablet, 2 Mobile */}
            ...
            {WALLPAPER_TYPES.map((type) => {
              const typeProductCount = products.filter(
                (p) =>
                  ...
                  (p.wallpaperType === type.slug || p.slug === type.productSlug)
              ).length;

              return (
                <Link
                  key={type.id}
                  href={`/categories/wallpapers/${type.slug}`}
      ...
      {/* ─── 3. SHOP ALL WALLPAPERS (Product Catalog Grid) ─── */}
      <section id="all-wallpapers" className="py-10 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header Bar */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 pb-4 border-b border-[#EAE4D8]">
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              <label htmlFor="wallpaper-sort" className="text-[11.5px] uppercase tracking-wider text-[#78716C] font-medium flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
            {sortedProducts.map((product) => {
              const matchedType = WALLPAPER_TYPES.find(
                (t) => t.slug === product.wallpaperType || t.productSlug === product.slug
              );
              const typeLabel = matchedType ? matchedType.name : 'Wallpapers & Wall Coverings';
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-between p-6 bg-gradient-to-b from-[#F7F4EE] to-[#EBE4D6] text-center">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-[11px] text-[#8C827A] tracking-wider uppercase">
                            Non-Woven Relief Embossing
                          </p>
                        </div>
                        <span className="text-[10px] text-[#78716C] italic">
                          Roll pattern updating
                        </span>
                      </div>
                    )}
