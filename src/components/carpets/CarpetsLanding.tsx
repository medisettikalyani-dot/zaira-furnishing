...
import { ChevronRight, ArrowRight, Heart, ArrowUpDown, Grid3X3 } from 'lucide-react';
import { CARPET_TYPES } from '@/lib/data/carpets';
import { Product } from '@/lib/data/types';

interface CarpetsLandingProps {
  products: Product[];
}

export function CarpetsLanding({ products }: CarpetsLandingProps) {
    ...
    <div className="bg-[#FAF7F2] min-h-screen text-[#1C1917] selection:bg-[#9A7B56] selection:text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-5 pb-2">
        <nav className="flex items-center gap-1.5 text-[12px] text-[#78716C]">
          ...
          <span className="text-[#1C1917] font-medium">Carpets & Rugs</span>
        </nav>
      </div>

      {/* ─── 2. SHOP BY CARPET & RUG TYPE (Reference Grid Structure) ─── */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-10">
            ...
            <p className="text-[12px] uppercase tracking-widest font-semibold text-[#9A7B56] mb-1">
              Shop by Carpet & Rug Type
            </p>
            <p className="text-[13.5px] sm:text-[14.5px] text-[#78716C]">
              Select an artisanal rug or broadloom carpet style below to view its tailored collection.
            </p>
          </div>
            ...
            {CARPET_TYPES.map((type) => {
              const typeProductCount = products.filter(
                (p) =>
                  ...
                  (p.carpetType === type.slug || p.slug === type.productSlug)
              ).length;

              return (
                <Link
                  key={type.id}
                  href={`/categories/carpets/${type.slug}`}
      ...
      {/* ─── 3. SHOP ALL CARPETS & RUGS (Product Catalog Grid) ─── */}
      <section id="all-carpets" className="py-10 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Header Bar */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
            {sortedProducts.map((product) => {
              const matchedType = CARPET_TYPES.find(
                (t) => t.slug === product.carpetType || t.productSlug === product.slug
              );
              const typeLabel = matchedType ? matchedType.name : 'Carpets & Rugs';
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-between p-6 bg-gradient-to-b from-[#F7F4EE] to-[#EBE4D6] text-center">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-[11px] text-[#8C827A] tracking-wider uppercase">
                            New Zealand Wool & Bamboo Silk
                          </p>
                        </div>
                        <span className="text-[10px] text-[#78716C] italic">
                          Artisanal weave preview updating
                        </span>
                      </div>
                    )}
